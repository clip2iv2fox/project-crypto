...
export const Loader: React.FC<LoaderProps> = ({
  ...
  const borderWidth = size === LoaderSize.l ? '6px' : size === LoaderSize.m ? '4.5px' : '3px';
  const height = size === LoaderSize.l ? '50px' : size === LoaderSize.m ? '30px' : '15px';
  const width = size === LoaderSize.l ? '50px' : size === LoaderSize.m ? '30px' : '15px';
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `${borderWidth} solid ${ className === 'b' ? 'rgba(52, 58, 64, 0.3)' : '#6C757D'}`,
        borderRadius: '50%',
